...
#import "Request_type_2VC.h"
@interface Request_typeVC ()<UITableViewDataSource,UITableViewDelegate>
...
-(void)loadMainView{
    ...
    //表示图
    {
        ...
        tableView.frame = CGRectMake(0, 0, WIDTH, HEIGHT - 49 - 64);
        [self.view addSubview:tableView];
        self.tableView = tableView;
        tableView.dataSource = self;
        tableView.delegate = self;
        tableView.rowHeight = (HEIGHT - 49 - 64)/10.0;
...
-(void)addTypeCallback{
    UIAlertController * ac = [UIAlertController alertControllerWithTitle:@"新加问题分类" message:@"此为一级分类，请填写分类名称" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction * action = [UIAlertAction actionWithTitle:@"确定" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction *action) {
        ...
        [send_dic setValue:@"0" forKey:@"parentId"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Request_type_2VC * vc = [Request_type_2VC new];
    vc.parent_id = request_type_id;
    vc.title = map[@"type_name"];
